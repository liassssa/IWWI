...
private void sendWebhook(String message) {
    ...
    try {
        java.net.URL webhook = new java.net.URL(url);
        java.net.HttpURLConnection connection = (java.net.HttpURLConnection) webhook.openConnection();
        ...
        """.formatted(username + " found a stash!", message, avatarUrl);
    ...
    public StashFinder() {
        super(IWWIAddon.CATEGORY, "IWWI-Finder", "Detects stashes and sends alerts.");
    }

    /**
     * Example event handling method.
     * Requires {@link IWWIAddon#getPackage()} to be setup correctly, otherwise the game will crash whenever the module is enabled.
     */
    ...
    private void onChunkData(ChunkDataEvent event) {
        ...
        if (chunk.getTotal() >= minimumStorageCount.get()) {
            sendWebhook("Found stash at **" + chunk.x + ", " + chunk.z + "** with " + chunk.getTotal() + " storage blocks.");
            mc.player.networkHandler.onDisconnect(new DisconnectS2CPacket(Text.literal("%s[%sYou Found A STASH%s] You found a STASH.".formatted(Formatting.GRAY, Formatting.BLUE, Formatting.GRAY))));
            toggle(); // Disable the StashFinder module after disconnecting
